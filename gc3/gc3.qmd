...
```{r full_grid_effects_EPTFUS}
full_grid <- read.csv("../data/data/prediction_grid.csv")

water_center <- attr(model_df$dist_to_water, "scaled:center")
water_scale <- attr(model_df$dist_to_water, "scaled:scale")
...
```{r}
y_preds = rowMeans(grid_predictions$p.y.predictive.samples)
full_grid$predictions <- y_preds

ggplot(data=full_grid) +
    # geom_sf(data = michiganplus) + 
    # facet_wrap(~ Species, ncol=3) + 
    geom_tile(aes(x=lon, y=lat, fill=predictions), width=0.25, height=0.25) +
...
```{r LANSOC}
eptfus_pred.covars <- cbind('intercept' = 1, dist_to_water=scaled_grid[, "water_dist"], "LASNOC"=0, "PERSUB"=0)
lansoc_pred.covars <- cbind('intercept' = 1, dist_to_water=scaled_grid[, "water_dist"], "LASNOC"=1, "PERSUB"=0)
persub_pred.covars <- cbind('intercept' = 1, dist_to_water=scaled_grid[, "water_dist"], "LASNOC"=0, "PERSUB"=1)

all_pred_covars <- rbind(eptfus_pred.covars, lansoc_pred.covars, persub_pred.covars)

all_coords <- rbind(
    scaled_gird[, c("lat", "lon")],
    scaled_gird[, c("lat", "lon")],
    scaled_gird[, c("lat", "lon")]
)

all_species <- c(
    rep("EPTFUS", nrow(scaled_grid)),
    rep("LASNOC", nrow(scaled_grid)),
    rep("PERSUB", nrow(scaled_grid))
)

grid_predictions <- spPredict(
    spatial_lm,
    pred.covars = all_pred_covars,
    pred.coords = all_coords,
    n.omp.threads = 8,
    start = 10000
)
```
