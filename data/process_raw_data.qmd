...
here::i_am("data/process_raw_data.qmd")
...
site_metadata <- read_csv(
    here("data/raw/Bat_Detector_Metadata/acoustic_sites.csv")
)
...
ggplot() +
    ...
    geom_point(aes(Long, Lat), site_metadata, color = "red", shape = "bullet") +
...
```{r}
site_metadata_sf <- st_as_sf(
    select(site_metadata, c(AcousticSite, Long, Lat)),
    coords = c("Long", "Lat"),
    crs = 4326 # Need to use WGS84 CRS
)
site_states <- site_metadata_sf %>%
...
ggplot() +
    ...
    geom_point(
        aes(Long, Lat),
        filter(site_metadata, is.na(state_province)),
...
```{r}
count(site_metadata, state_province, sort = TRUE, name = "num_sites")
```
...
```{r}
great_lakes_site_metadata <- filter(
    site_metadata,
    !(
...
```{r}
great_lakes_site_data <- filter(
    site_data, AcousticSite %in% great_lakes_site_metadata$AcousticSite
)
```
...
c(
    num_sites_reduc = 100 * (
        nrow(great_lakes_site_metadata) / nrow(site_metadata) - 1
    ),
    num_data_recs_reduc = 100 * (
        nrow(great_lakes_site_data) / nrow(site_data) - 1
    )
)
```
...
env <- list2env(
    list(
        site_metadata = great_lakes_site_metadata,
        site_data = great_lakes_site_data
    )    
)
save(
    list = ls(env),
    file = here("data/processed/site_data_metadata.RData"),
    envir = env
)
```
