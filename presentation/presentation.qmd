...
## Base Non-Spatial Poisson Model

All spatial Poisson models were based on the non-spatial Poisson model

$$
Y_k \mid X_k = x_k \sim \text{Poisson}(n_k\exp(\beta_0 + x_k^{\top}\beta))
$$
...
- $X_k$ is the covariate vector and $x_k$ is its observed value
- $\beta_0$, $\beta$ contain parameters to be estimated
